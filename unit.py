...
MAP_HEIGHT=1000 #почему глобальные переменные? Это всё определено в Basic классе, оттуда и надо вызывать 
MAP_WIDTH= 100
GRND_BLOCK_H=64 #Что это вообще такое?

player_controlled=1 
ai_controlled=2

class Unit(pygame.sprite.Sprite): #Должен наследоваться от movableobject
    ...
    def _update(self):
        DAMPING = 0.3 #почему классова перменная доступна откуда угодно?
...
class Chaser(Unit): #откуда в юните охотник? Почему он наследует от Unit а не от Player?
    def __init__(self, game, team):
